# --- START OF COMPLETE backend/main.py FILE ---
...
app = FastAPI()
# Initialize the python-telegram-bot application to send push notifications
...
def validate_init_data(init_data: str, bot_token: str) -> dict | None:
    try:
        # The data is URL-encoded, so we need to decode it
...
async def get_current_user(request: Request) -> ValidatedUser:
    """A FastAPI dependency that validates Telegram InitData and returns the user's data."""
    ...
    # Check for our simple, foolproof keyword in query_id for local development
    if parsed_init_data.get("query_id") == "MOCK_FOR_LOCAL_DEV":
        ...
        # This is the original, secure validation for production
        user_data = validate_init_data(init_data, BOT_TOKEN)
        if user_data is None:
            raise HTTPException(status_code=403, detail="Invalid Telegram InitData")
    
    # Ensure user exists in our system and update their activity
...
async def get_my_profile(current_user: ValidatedUser = Depends(get_current_user)):
    ...
    if not profile:
        raise HTTPException(status_code=404, detail="User profile not found after validation.")
...
async def get_user_profile(target_uid: str, current_user: ValidatedUser = Depends(get_current_user)):
    ...
    target_profile["relation"] = {
        ...
        "is_friend": is_friend(my_uid, target_uid),
...
async def get_chat_list(current_user: ValidatedUser = Depends(get_current_user)):
    ...
    previews = [{
        "partner_uid": p_uid,
        "partner_username": get_username_by_uid(p_uid),
...
async def get_full_chat(partner_uid: str, current_user: ValidatedUser = Depends(get_current_user)):
    user_history = tid_to_history_cache.get(current_user.id, {})
    messages = user_history.get(partner_uid, [])
    for msg in messages:
...
async def delete_chat_history(partner_uid: str, current_user: ValidatedUser = Depends(get_current_user)):
    user_history = tid_to_history_cache.get(current_user.id)
    if user_history and partner_uid in user_history:
        del user_history[partner_uid]
...
async def send_message(partner_uid: str, payload: MessagePayload, current_user: ValidatedUser = Depends(get_current_user)):
    ...
    if str(partner_uid) in get_blocked_users_by_tid(current_user.id): raise HTTPException(status_code=403, detail="You must unblock this user to send a message.")
...
async def get_friends_list(current_user: ValidatedUser = Depends(get_current_user)):
    my_uid = get_unique_id_by_tid(current_user.id)
    friends_uids = get_friends_by_uid(my_uid)
...
async def get_requests_list(current_user: ValidatedUser = Depends(get_current_user)):
    my_profile = get_user_profile_by_tid(current_user.id)
    if not my_profile: raise HTTPException(status_code=404, detail="Current user profile not found.")
    return JSONResponse(content={
        "received": [get_user_profile_by_uid(uid) for uid in my_profile.get("received_requests", []) if get_user_profile_by_uid(uid)],
        "sent": [get_user_profile_by_uid(uid) for uid in my_profile.get("sent_requests", []) if get_user_profile_by_uid(uid)]
    })
...
async def handle_user_action(action: str, target_uid: str, current_user: ValidatedUser = Depends(get_current_user)):
    my_tid, my_uid, target_tid = current_user.id, get_unique_id_by_tid(current_user.id), get_telegram_id_by_uid(target_uid)
    ...
    if action == "add_friend":
        ...
        if is_friend(my_uid, target_uid) or has_sent_request(my_uid, target_uid): raise HTTPException(status_code=400, detail="Already friends or request sent.")
        s_profile.setdefault("sent_requests", []).append(target_uid)
        r_profile.setdefault("received_requests", []).append(my_uid)
        if target_tid:
            await send_push_notification(ptb_app.bot, target_tid, f"🤝 You have a new friend request from *{telegram.helpers.escape_markdown(current_user.username or current_user.first_name, version=2)}*\\!")
    elif action == "accept_friend":
        add_friend_by_uid(my_uid, target_uid); add_friend_by_uid(target_uid, my_uid)
        remove_pending_request(sender_uid=target_uid, receiver_uid=my_uid)
        if target_tid:
            await send_push_notification(ptb_app.bot, target_tid, f"🎉 Your friend request was accepted by *{telegram.helpers.escape_markdown(current_user.username or current_user.first_name, version=2)}*\\!")
    elif action == "decline_friend": remove_pending_request(sender_uid=target_uid, receiver_uid=my_uid)
    elif action == "cancel_request": remove_pending_request(sender_uid=my_uid, receiver_uid=target_uid)
    ...
    return JSONResponse(content={"status": f"Action '{action}' successful."})
...
async def react_to_last_message(partner_uid: str, emoji: str, current_user: ValidatedUser = Depends(get_current_user)):
    ...
    my_history = tid_to_history_cache.get(my_tid, {}).get(partner_uid, [])
    if not my_history: raise HTTPException(status_code=404, detail="Chat history not found.")

    message_to_react = my_history[-1]
    reactions = message_to_react.setdefault('reactions', [])
    ...
    if partner_tid and (p_history := tid_to_history_cache.get(partner_tid, {}).get(my_uid, [])):
        p_message = next((msg for msg in reversed(p_history) if msg.get("timestamp") == message_to_react.get("timestamp")), None)
        if p_message: p_message['reactions'] = reactions
...
# --- END OF COMPLETE backend/main.py FILE ---
